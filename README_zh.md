...
- **Load Image 按钮** — 通过节点按钮或向预览区域拖放文件来加载图像。自动从图像生成 Layer 1（灰度→Alpha 转换）。加载新图像时清除现有蒙版图层
- **节点预览** — 始终显示（固定 160 px）。预览背景图像或应用后的蒙版。未选择图像时作为文件拖放区使用
- **图像/蒙版切换** — 在节点上切换图像预览和蒙版预览。加载新图像时自动切换为图像显示
...
通过 ComfyUI Manager 的「Install via Git URL」或「Custom Nodes Manager」输入以下 URL 安装：

```
https://github.com/ketle-man/comfyui-mask-editor-one
```

或在 [Comfy Registry](https://registry.comfy.org/publishers/statsu/nodes/comfyui-mask-editor-one) 搜索 **Mask Editor One** 进行安装。
...
2. 点击 `Load Image` 按钮或向预览区域拖放图像文件以加载背景图像
...
| 操作 | 效果 |
|------|------|
| 点击 `Load Image` | 打开文件选择对话框 |
| 拖放到预览区域 | 将图像文件拖放到节点预览区域进行加载 |
| 点击 `👁 显示: 蒙版` | 将节点预览切换为蒙版显示 |
| 点击 `👁 显示: 图像` | 将节点预览切换为图像显示 |

- 加载图像时清除所有现有蒙版图层，并从图像灰度自动生成 Layer 1
- 加载图像后预览自动切换为图像显示
- Apply 后自动切换为蒙版预览，节点上显示黑白蒙版
- 编辑器**打开时**更换背景请使用编辑器页脚的 `Image` 拖放区
